...
import {
  createInitialState, applyAction, movesFrom, affordableMoves,
  VALUE, NAME_KO, GLYPH, fieldAt,
} from './engine.js';
...
function renderBoard(s, mc) {
  ...
  let legal = [];
  if (selected) legal = movesFrom(s, selected[0], selected[1]);
...
function renderTrade(s, mc) {
  ...
  if (s.pending && s.pending.from !== mc && (s.seats.w === net.id || s.seats.b === net.id)) {
    ...
    div.innerHTML = `<b>거래 제안</b><br>상대가 에너지 ${p.offerEnergy} 제공 / 에너지 ${p.wantEnergy} 요구
      ${p.offerPiece ? '<br>+ 기물 양도' : ''}${p.wantPiece ? '<br>+ 기물 요구' : ''}`;
  ...
  if (s.justLandedTrade && s.justLandedTrade === mc && !s.pending) {
    const div = document.createElement('div');
    div.innerHTML = `<div class="banner info">💱 거래소 — 상대에게 에너지 거래를 제안하세요.</div>
      <label>내가 줄 에너지</label><input type="number" id="offE" value="0" min="0" />
      <label>내가 받을 에너지</label><input type="number" id="wantE" value="0" min="0" />`;
    const btn = document.createElement('button'); btn.className = 'primary'; btn.style.marginTop = '8px'; btn.textContent = '제안 보내기';
    btn.onclick = () => {
      net.dispatch({
